...
@ActivityScoped
class CategoryStoreAdapter : RecyclerView.Adapter<CategoryStoreAdapter.ViewHolder> {

    private var layoutInflater: LayoutInflater? = null;
    var clickAdapterCategory: ClickAdapterCategoryStore? = null;
...
//    lateinit var binding:AdapterCategoryBinding
    ...
    constructor(@ActivityContext context: Context) {
        ...
        list = ArrayList<CategoryStoreTO>();

    }

    fun updateAdapter(list: List<CategoryStoreTO>?, type: String?) {
...
//        }
        ...
        holder.binding.nameCategory.text = if (prefLang.language == ContextApp.EN)
            list?.get(position)?.categoryStoreName ?: ""
        else
            list?.get(position)?.categoryStoreName ?: ""

        holder.itemView.setOnClickListener {
            if (clickAdapterCategory != null)
                clickAdapterCategory?.OnItemClickListenerAdapter(list?.get(position)!!, binding = holder.binding)
        }
